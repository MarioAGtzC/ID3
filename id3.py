...
class DecisionTree:
	def read_data(self, filename):
		...
		self.goalName = self.get_goal(data)
		data = data[1:]
		self.classes = self.get_classes(data)
		data = self.get_pure_data(data)

		return data,self.classes,self.featureNames,self.goalName	
...
tree = DecisionTree()
tr_data, clss, attrs, goal = tree.read_data('test.csv')

tree1 = tree.create_tree(tr_data, clss, attrs)

print("Decision tree to check:", goal)
